...
interface TransformationPanelProps {
  ...
  selectedDestination?: "YouTube Shorts" | "TikTok";
  selectedContentIds?: number[];
}

export function TransformationPanel({ onTransformSuccess, selectedDestination, selectedContentIds }: TransformationPanelProps) {
  const { showToast } = useContext(ToasterContext);
  const queryClient = useQueryClient();
  
  const [selectedItems, setSelectedItems] = useState<number[]>([]);
  const [targetPlatform, setTargetPlatform] = useState<"YouTube Shorts" | "TikTok">(selectedDestination || "YouTube Shorts");
      ...
      <CardContent className="px-6 py-5">
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900">No content imported</h3>
            <p className="mt-1 text-sm text-gray-500">Import content from Instagram to start transforming.</p>
          </div>
        ) : (
          <div>
            {contentItems
                  ...
                  <div className="h-10 w-10 flex-shrink-0 bg-gray-200 rounded overflow-hidden">
                    <img 
                      ...
                      onError={(e) => {
                        ...
                        (e.target as HTMLImageElement).src = 'https://placehold.co/80x80/gray/white?text=No+Image';
                      }}
                    />
                  </div>
                  <div className="ml-3">
            ...
            {!selectedDestination && (
              <div className="mt-6 mb-6">
                <h4 className="text-sm font-medium text-gray-900 mb-4">Destination Platform</h4>
                
                <RadioGroup 
                  value={targetPlatform}
                  onValueChange={(value) => setTargetPlatform(value as "YouTube Shorts" | "TikTok")}
                  className="flex flex-col space-y-4"
                >
                  <div className="flex items-start space-x-4">
                    <RadioGroupItem value="YouTube Shorts" id="youtube-shorts" className="mt-1" />
                    <div className="flex-1 border rounded-lg p-4 cursor-pointer hover:border-primary transition-colors" onClick={() => setTargetPlatform("YouTube Shorts")}>
                      <div className="flex items-center">
                        <SiYoutube className="h-5 w-5 text-red-600 mr-2" />
                        <Label htmlFor="youtube-shorts" className="font-medium cursor-pointer">
                          YouTube Shorts
                        </Label>
                      </div>
                      <p className="text-sm text-gray-500 mt-2">
                        Optimize your content for YouTube Shorts vertical video format with auto-captions
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1 border rounded-lg p-4 cursor-pointer hover:border-primary transition-colors" onClick={() => setTargetPlatform("TikTok")}>
                      <div className="flex items-center">
                      ...
                      <p className="text-sm text-gray-500 mt-2">
                        Optimize your content for TikTok with trending sounds and effects
                      </p>
                    </div>
                  </div>
                </RadioGroup>
            ...
            {selectedDestination && (
              <div className="mt-6 mb-6">
                <h4 className="text-sm font-medium text-gray-900 mb-3">Selected Destination</h4>
            ...
            <div className="mt-6">
              <h4 className="text-sm font-medium text-gray-900 mb-3">Transformation Options</h4>
              ...
              <div className="space-y-3">
                <div className="flex items-center">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => 
                      setTransformationOptions(prev => ({ ...prev, aspectRatio: !!checked }))
                    }
                  />
                ...
                <div className="flex items-center">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => 
                      setTransformationOptions(prev => ({ ...prev, autoCaption: !!checked }))
                    }
                  />
                ...
                <div className="flex items-center">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => 
                      setTransformationOptions(prev => ({ ...prev, branding: !!checked }))
                    }
                  />
                  <Label htmlFor="add-branding" className="ml-2 text-sm text-gray-700">
                ...
                <div className="flex items-center">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => 
                      setTransformationOptions(prev => ({ ...prev, optimizeTitles: !!checked }))
                    }
                  />
                  <Label htmlFor="optimize-titles" className="ml-2 text-sm text-gray-700">
                    Optimize titles for {targetPlatform === "YouTube Shorts" ? "YouTube" : "TikTok"} algorithm
                  </Label>
                </div>
              </div>
